...
# git logs
fgl() {
  git log --oneline | fzf --preview 'git log {1} --stat -p'
}
fshow() {
...
source /Users/wuxiaoming/local/softwares/sh/git/forgit-master/forgit.plugin.zsh
forgit_checkout_commit=gcoo
...
source /Users/wuxiaoming/local/softwares/sh/zsh/zsh-interactive-cd-master/zsh-interactive-cd.plugin.zsh
function cdd() {
    if [[ "$#" != 0 ]]; then
        builtin cd "$@";
        return
...
# z
source /Users/wuxiaoming/local/softwares/sh/zsh/z-master/z.sh
